...
class Pair:
    ...
    def tokenize(self, tokenizer: AutoTokenizer, device: str="cpu") -> Tokenized:
        """Tokenize the pair and produce alignments using character-level mapping.
        
        This approach tokenizes the full sentence first and uses offset_mapping
        to properly align tokens to spans, handling tokenizers that merge whitespace.
        """
        # Tokenize full sentences with offset mapping
        base_text = ''.join(self.base)
        src_text = ''.join(self.src)
        
        base_tok = tokenizer(base_text, return_tensors="pt", padding=True, return_offsets_mapping=True).to(device)
        src_tok = tokenizer(src_text, return_tensors="pt", padding=True, return_offsets_mapping=True).to(device)
        
        # Extract offset mappings (char start, char end for each token)
        base_offsets = base_tok.pop("offset_mapping")[0].tolist()  # Remove from dict, keep as list
        src_offsets = src_tok.pop("offset_mapping")[0].tolist()
        
        # Compute character boundaries for each span
        def get_span_boundaries(spans):
            boundaries = []
            char_pos = 0
            for span in spans:
                boundaries.append((char_pos, char_pos + len(span)))
                char_pos += len(span)
            return boundaries
        
        base_boundaries = get_span_boundaries(self.base)
        src_boundaries = get_span_boundaries(self.src)
        
        # Map tokens to spans using character positions
        def compute_alignment(offsets, boundaries):
            alignment = [[] for _ in boundaries]
            for token_idx, (tok_start, tok_end) in enumerate(offsets):
                # Skip special tokens (offset 0,0)
                if tok_start == 0 and tok_end == 0:
                    continue
                # Find which span this token belongs to
                for span_idx, (span_start, span_end) in enumerate(boundaries):
                    # Token overlaps with span if token starts before span ends and ends after span starts
                    if tok_start < span_end and tok_end > span_start:
                        alignment[span_idx].append(token_idx)
                        break  # Each token belongs to one span
            return alignment
        
        alignment_base = compute_alignment(base_offsets, base_boundaries)
        alignment_src = compute_alignment(src_offsets, src_boundaries)
...
class Batch:
    ...
    def __init__(self, pairs: list[Pair], tokenizer: AutoTokenizer, device: str="cpu"):
        ...
        # labels - use add_special_tokens=False and take LAST token (not first)
        # because BabyLM tokenizes ' him' as [space_token, word_token]
        self.base_labels = torch.LongTensor([tokenizer.encode(pair.base_label, add_special_tokens=False)[-1] for pair in pairs]).to(device)
        self.src_labels = torch.LongTensor([tokenizer.encode(pair.src_label, add_special_tokens=False)[-1] for pair in pairs]).to(device)
...
class Dataset:
    ...
    def __init__(self, data: dict):
        ...
        self.template = [x for x in re.split(r"(?<=\})|(?= \{)|(?<! )(?=\{)", '' + random.choice(self.templates)) if x != '']
